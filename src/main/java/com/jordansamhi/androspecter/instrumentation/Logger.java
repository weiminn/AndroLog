...
public class Logger {
     ...
     */
    public Tuple logAllMethods(String tagToLog, boolean includeLibraries) {
        ArrayList<SootClass> allClasses = new ArrayList<>();
        int all_methods_count = 0;
        int app_methods_count = 0;
        allClasses.addAll(Scene.v().getApplicationClasses());
        for (SootClass c: allClasses){
            if (c.getName().equals(Constants.LOG_CHECKER_CLASS)) {
                continue;
            }
            all_methods_count += c.getMethodCount();
            if (LibrariesManager.v().isLibrary(c)) {
                continue;
            }
            app_methods_count += c.getMethodCount();
        }
        ...
        // this.counts = new Tuple(all_methods_count, app_methods_count);
        // return this.counts;

        return new Tuple(all_methods_count, app_methods_count);
    }
